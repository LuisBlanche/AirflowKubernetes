...
with DAG(dag_id=dag_id, default_args=default_args, schedule_interval=None, max_active_runs=1) as dag:
    ...
    impute_train_data = WrapperKubernetesPodOperator(
        ...
        cmds=[
            ...
            "{{task_instance.xcom_pull(task_ids='make_dataset', key='train_features_path')}}",
    ...
    impute_unseen_data = WrapperKubernetesPodOperator(
        ...
        cmds=[
            "python",
            "/app/potability/data/impute.py",
            "{{task_instance.xcom_pull(task_ids='make_dataset', key='unseen_features_path')}}",
            f"{DATA_PATH}/processed/train_features_imputed.csv",
        ],
        dag=dag,
        do_xcom_push=True,
    ...
    build_train_features = WrapperKubernetesPodOperator(
        ...
        cmds=[
            ...
            "{{task_instance.xcom_pull(task_ids='impute_train_data', key='imputed_path')}}",
    ...
    build_unseen_features = WrapperKubernetesPodOperator(
        ...
        cmds=[
            "python",
            "/app/potability/features/build_features.py",
            "{{task_instance.xcom_pull(task_ids='impute_unseen_data', key='imputed_path')}}",
    ...
    train_rf = WrapperKubernetesPodOperator(
        ...
        cmds=[
            ...
            "rf",
            "{{task_instance.xcom_pull(task_ids='build_train_features', key='features_path')}}",
            "{{task_instance.xcom_pull(task_ids='make_dataset', key='train_target_path')}}",
            MODELS_PATH,
            N_ITER,
            N_JOBS,
            N_CV,
    ...
    predict_rf = WrapperKubernetesPodOperator(
        ...
        cmds=[
            ...
            "{{task_instance.xcom_pull(task_ids='build_unseen_features', key='features_path')}}",
            "{{task_instance.xcom_pull(task_ids='make_dataset', key='unseen_target_path')}}",
